...
int main(int argc, char *argv[])
{
	...
	// Redirect stdout to a file to verify mdbuf
	freopen("mdfile.txt", "w", stdout);
	...
	RSA* rsapriv = PEM_read_bio_RSAPrivateKey(hash,NULL,0,NULL);
	
	
	int privateEncrypt = 
	  RSA_private_encrypt(mdlen, 
			      (const unsigned char*) mdbuf[0],
			      (unsigned char*) mdbuf[mdlen-1], 
			      rsapriv,
			      RSA_PKCS1_PADDING);
	
	
	//RSA* rsapub = PEM_read_bio_RSA_PUBKEY(hash,
	//			      rsapriv,
	//			      0);   //privateEncrypt);
